  ...
  home = rec {
    ...
    create-homes =
      ...
      let
        ...
        # Helper function to find base home configuration for a user
        find-base-home =
          home-metadata:
          let
            user-metadata = split-user-and-host home-metadata.name;
            # Only look for base if this home has a hostname specified
            has-hostname = user-metadata.host != "";
            base-name = "${user-metadata.user}@${home-metadata.system}";
            # Find the base home in the metadata list
            base-home = builtins.filter (h: h.name == base-name) target-homes-metadata;
          in
          if has-hostname && (builtins.length base-home) > 0 then builtins.head base-home else null;
        ...
        create-home' =
          ...
          let
            ...
            # Create a wrapper module that imports base first if it exists
            # This ensures the base configuration is loaded first and can be overridden
            path-with-base =
              if base-home != null then
                {
                  imports = [
                    base-home.path
                    home-metadata.path
          ...
          in
          {
            "${name}" = create-home (
              ...
              // {
                path = path-with-base;
                # Module order: user-specific modules, global modules
                # These come after the path modules and can override both base and host-specific settings
      ...
      let
        ...
        system-modules = builtins.map (
          ...
          let
            host-matches =
              (name == "${user-name}@${host}")
              || (if config.snowfallorg.useGlobalUsers then (name == "${user-name}@${system}") else false);
